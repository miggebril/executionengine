...
sql = "select mgt_member + mgt_group + mgt_trader, acct_name"
sql = sql & " from tt_mgt "
sql = sql & " inner join tt_account on tt_account.acct_mgt_id = tt_mgt.mgt_id"
sql = sql & " where mgt_publish_to_guardian = 1"
sql = sql & " order by mgt_member + mgt_group + mgt_trader, acct_name"
...
sql = "select * from tt_mgt LEFT JOIN tt_user ON tt_mgt.mgt_last_updated_user_id = tt_user.user_id order by 1"

set rs = conn.execute(sql)

do while not rs.eof
	mgt_key = rs("mgt_member") & rs("mgt_group") & rs("mgt_trader")
	if accounts_dict.Exists(mgt_key) then
		...
		output_file.Write rs("mgt_description")
		output_file.Write "|"

		output_file.Write rs("mgt_credit")
		output_file.Write "|"

		output_file.Write rs("mgt_currency")
		output_file.Write "|"

		if rs("mgt_allow_trading") = 1 then
		...
		output_file.Write my_format_date(rs("mgt_last_updated_datetime"))		
		output_file.Write "|"
...
sql = "SELECT tt_product_limit.*, tt_user.user_login, tt_gateway.gateway_name, tt_product_type.product_description, mgt_member, mgt_group, mgt_trader"
sql = sql & " FROM tt_mgt "
...
sql = sql & " ON tt_mgt.mgt_id = tt_product_limit.plim_mgt_id "
sql = sql & " WHERE tt_mgt.mgt_publish_to_guardian = 1"
sql = sql & " ORDER BY mgt_member, mgt_group, mgt_trader, gateway_name, plim_product, product_description "
...
'	set ts = fs.OpenTextFile(config_folder + "\\TTUserSetupServer_720_DEBUG.ini")
